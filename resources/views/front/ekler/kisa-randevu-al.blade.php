<!-- appointment area start -->
<section class="appointment__area pb-50">
    <div class="container">
        <div class="appointment__area-inner white-bg p-relative">
            <div class="row">
                <div class="col-xl-5 col-lg-5">
                    <div class="appointment__form-wrapper">
                        <h1>Randevu Al</h1>
                        <div class="appointment__form-inner">
                            <form action="#">
                                <input type="text" placeholder="Adınız Soyadınız">
                                <input type="text" placeholder="Telefon Numaranız">
                                <textarea name="app-txt" cols="30" rows="10" placeholder="Talep Etiğiniz Terapi"></textarea>

                                <button class="sub-btn" type="submit">Randevu Oluştur</button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="col-xl-7 col-lg-7">
                    <div class="appointment__counter">
                        <div class="section__title mb-60">
                            <div class="section__icon mb-20">
                                <img src="assets/img/icon/sec-icon-1.png" alt=""><span
                                    class="section__sub-title ml-20">WPSİKOLOJİ Başarımız</span>
                            </div>
                            <h1>En Çok Terapi Desteğinde Bulunan Terapilermiz</h1>
                        </div>
                        <div class="appointment__counter-item mr-60">
                            <div class="row no-gutters">
                                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6">
                                    <div class="single-counter pb-25 pl-25">
                                        <h3 class="pink-color"> <span class="counter">120</span></h3>
                                        <span>Çift terapisi</span>
                                    </div>
                                </div>
                                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6">
                                    <div class="single-counter pb-25 pl-65">
                                        <h3><span class="counter green-color">215</span></h3>
                                        <span>Depresyon Terapisi</span>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 ">
                                    <div class="single-counter pt-20 pl-25 ">
                                        <h3><span class="counter green-color">315</span>+</h3>
                                        <span>Flört & İlişki Terapisi</span>
                                    </div>
                                </div>
                                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6">
                                    <div class="single-counter pt-20 pl-65">
                                        <h3 class="pink-color"><span class="counter">125</span> </h3>
                                        <span>Aile Danışmanlığı</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- appointment area end -->